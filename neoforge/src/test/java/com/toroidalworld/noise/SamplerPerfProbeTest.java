...
class SamplerPerfProbeTest {
    ...
    void measuresSamplerPaths() {
        ...
        report.append(measure("wrapped, undeclared (pre-fix path), floored",
                (x, z) -> PeriodicNoiseSampler.sample(this.permutations, this.xo, this.yo, this.zo,
                        WORLD, periodOneScale, x, 0.0, z, 0.0, 0.0, -1.0)));
        report.append(measure("wrapped, corrected flat, floored (worst case)",
                (x, z) -> PeriodicNoiseSampler.sample(this.permutations, this.xo, this.yo, this.zo,
                        WORLD, periodOneScale, x, 0.0, z, 0.0, 0.0, 0.0)));
        report.append(measure("wrapped, corrected live, floored (liveness lookup)",
                (x, z) -> PeriodicNoiseSampler.sample(this.permutations, this.xo, this.yo, this.zo,
                        WORLD, periodOneScale, x, 0.0, z, 0.0, 0.0, 1.0)));
        report.append(measure("wrapped, corrected, period 32 (fast path)",
                (x, z) -> PeriodicNoiseSampler.sample(this.permutations, this.xo, this.yo, this.zo,
                        WORLD, fastPathScale, x, 0.0, z, 0.0, 0.0, 0.0)));
