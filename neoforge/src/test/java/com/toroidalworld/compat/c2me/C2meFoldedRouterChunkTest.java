...
class C2meFoldedRouterChunkTest {
    // 1.21.1 has no NO_BEARDS, and DensityFunctions.BeardifierMarker is not reachable from here. The probe
    // wants no beards at all, which is the marker's own zero body.
    private static final DensityFunctions.BeardifierOrMarker NO_BEARDS = new DensityFunctions.BeardifierOrMarker() {
        @Override
        ...
        @Override
        public void fillArray(double[] values, DensityFunction.ContextProvider contexts) {
    ...
    private static Terrain fillOneChunk(RandomState state) {
        ...
        InterpolatedNoiseChunk noiseChunk = new InterpolatedNoiseChunk(cellCountXZ, state,
                chunkStartBlockX, chunkStartBlockZ, noiseSettings, NO_BEARDS, overworld,
