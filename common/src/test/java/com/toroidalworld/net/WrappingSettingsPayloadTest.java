...
class WrappingSettingsPayloadTest {
    ...
    private static final ResourceKey<Level> DATAPACK_DIMENSION =
            ResourceKey.create(Registries.DIMENSION, ResourceLocation.fromNamespaceAndPath("cject", "hollow"));
    ...
    private static byte[] dimensionWire(ResourceKey<Level> dimension) {
        byte[] name = dimension.location().toString().getBytes(StandardCharsets.UTF_8);
