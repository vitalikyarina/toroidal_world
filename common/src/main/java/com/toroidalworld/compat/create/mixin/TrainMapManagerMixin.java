...
public abstract class TrainMapManagerMixin {
    ...
    private static Vec3 toroidal$canonicaliseCarriage(TrainMapSync.TrainMapSyncEntry entry, int carriageIndex,
        ...
        if (firstBogey) {
            return TrainMapViewFold.canonical(raw);
        }

        Vec3 leading = TrainMapViewFold.canonical(original.call(entry, carriageIndex, true, time));
        return TrainMapViewFold.nearestTo(leading, raw);
    }
