...
// Two of the four readers of a distance bound, one arithmetic (see SeamDistanceBounds): each hands DistancePredicate
// two absolute positions and the predicate subtracts them raw. A kill five blocks away through the seam then measures
...
//
// Written on the call rather than on the method it sits in, because both of these read the same position again
...
//
// The third reader — the `distance` field every EntityPredicate carries for loot tables and predicate files — lives in
// EntityPredicateDistanceMixin: its matches() has a second overload, so it needs an exact descriptor of its own.
@Mixin({
