...
//
// Client-only by use, not by annotation: the one caller is the payload handler's lambda body, which a dedicated
// server registers but never runs, so this class never loads there.
