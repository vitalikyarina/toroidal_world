...
A world's shape lives entirely in its chunk generators, so a dedicated server needs no screen: the mod ships the same named world sizes the create-world screen offers as world presets, and one `server.properties` line picks one. The result is a fully toroidal world — overworld, nether and End — that behaves exactly like one created in singleplayer. Joining players need the mod installed; they receive the world's bounds automatically on login.
...
The five presets match the create-world screen exactly:

| `level-type` | Overworld | Nether (portal scale) | End |
| --- | --- | --- | --- |
| `toroidal_world:tiny` | 32 chunks (512 blocks) | 16 chunks (256 blocks), 1:2 | 256 chunks (4096 blocks) |
| `toroidal_world:small` | 64 chunks (1024 blocks) | 16 chunks (256 blocks), 1:4 | 320 chunks (5120 blocks) |
| `toroidal_world:medium` | 128 chunks (2048 blocks) | 16 chunks (256 blocks), 1:8 | 384 chunks (6144 blocks) |
| `toroidal_world:large` | 256 chunks (4096 blocks) | 32 chunks (512 blocks), 1:8 | 448 chunks (7168 blocks) |
| `toroidal_world:huge` | 512 chunks (8192 blocks) | 64 chunks (1024 blocks), 1:8 | 512 chunks (8192 blocks) |
...
- **Overworld** — both axes the same width, at least 16 chunks (256 blocks). Center the range on zero as the example does (`min_chunk = -width/2`).
- **Nether** — the overworld width divided by the portal scale, and the scale must divide it exactly; the nether itself must stay at least 16 chunks (256 blocks) wide. Vanilla's 1:8 portal ratio therefore needs an overworld of at least 128 chunks (2048 blocks). An uneven ratio breaks portal linking near the seam — this rule is not optional.
- **End** — independent of the other two, at least 192 chunks (3072 blocks). Smaller Ends lose the outer island ring — no end cities, no elytra — and let gateway teleports reach across the seam.
...
- `level-type` matters only at world creation. On an existing world it changes nothing; to get a toroidal world, start from a fresh `world` folder.
